...
RadioMedium::RadioCacheEntry::RadioCacheEntry() :
    receptionIntervals(NULL)
{
}
...
RadioMedium::~RadioMedium()
{
    ...
    for (std::vector<RadioCacheEntry>::const_iterator it = radioCache.begin(); it != radioCache.end(); it++) {
        const RadioCacheEntry& radioCacheEntry = *it;
...
RadioMedium::RadioCacheEntry *RadioMedium::getRadioCacheEntry(const IRadio *radio) const
{
...
const IReceptionDecision *RadioMedium::computeReceptionDecision(const IRadio *radio, const IListening *listening, const ITransmission *transmission, const std::vector<const ITransmission *> *transmissions) const
{
    ...
    const IReceptionDecision *decision = radio->getReceiver()->computeReceptionDecision(listening, reception, interference, getSNIR(radio, transmission));
    return decision;
}
